...
export function METClock() {
  const [elapsed, setElapsed] = useState(getMissionElapsed())
  ...
  return (
    <div className="rounded-xl bg-zinc-900/70 border border-zinc-700/40 p-5 backdrop-blur-sm">
      ...
      <div className="flex items-baseline gap-1 font-mono">
        ...
        <span className="text-4xl font-bold text-white">{pad(elapsed.hours)}</span>
        <span className="text-lg text-zinc-400 mr-2">h</span>
        <span className="text-4xl font-bold text-white">{pad(elapsed.minutes)}</span>
        <span className="text-lg text-zinc-400 mr-2">m</span>
        <span className="text-4xl font-bold text-white">{pad(elapsed.seconds)}</span>
